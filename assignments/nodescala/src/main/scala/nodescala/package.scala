...
package object nodescala {
  ...
  //needed to move outside value class to use async
  def allhelper[T](fs: List[Future[T]]): Future[List[T]] = async{
    fs match {
      case Nil => Nil
      case (ft::fts) => await(ft) :: await(allhelper(fts))
    }
  }
  ...
  implicit class FutureCompanionOps(val f: Future.type) extends AnyVal {
    ...
    def all[T](fs: List[Future[T]]): Future[List[T]] = allhelper(fs)


    def all2[T](fs: List[Future[T]]): Future[List[T]] = {
      val ps = Promise[List[T]]()
      ps.success(Nil)

      fs.foldRight(ps.future) {
...
//    }
    ...
    //doesn't work for failures ?
    def continueWith2[S](cont: Future[T] => S): Future[S] =
      f map (t => cont(Future(t)))

    def continueWith[S](cont: Future[T] => S): Future[S] = {
      val p = Promise[S]()
      f onComplete {
        case result => p.complete(Try(cont(f)))
      }
      p.future
    }
...
//    }
    ...
    //doesn't work for failures ?
    def continue2[S](cont: Try[T] => S): Future[S] =
      f map (t => cont(Try(t)))

    def continue[S](cont: Try[T] => S): Future[S] = {
      val p = Promise[S]()
      f onComplete {
        case result => p.complete(Try(cont(result)))
      }
      p.future
